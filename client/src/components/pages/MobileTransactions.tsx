      ...
      {showDetailModal && selectedPaymentDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-xl w-full max-w-6xl my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4 sticky top-0 bg-white dark:bg-gray-800 pb-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Transaction Details - {selectedPaymentDetail.service_request.reference_id}</h2>
            ...
            <div className="mb-6">
              <h3 className="font-semibold text-lg text-gray-800 dark:text-white mb-3">Payment Gateway Transactions</h3>
              {selectedPaymentDetail.payment_gateway_transactions.length > 0 ? (
                <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                  <table className="w-full">
                    <thead className="bg-gray-100 dark:bg-gray-700">
                      <tr>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Txn ID</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Amount</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Mode</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Bank</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Status</th>
                        <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Date</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedPaymentDetail.payment_gateway_transactions.map((pgTxn, index) => (
                        <tr key={index} className="border-b border-gray-100 dark:border-gray-800">
                          <td className="px-3 py-2 text-sm font-mono text-gray-600 dark:text-gray-300">{pgTxn.client_txn_id}</td>
                          <td className="px-3 py-2 text-sm font-semibold text-gray-900 dark:text-white">₹{parseFloat(pgTxn.amount).toLocaleString()}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{pgTxn.payment_mode || '-'}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{pgTxn.bank_name || '-'}</td>
                          <td className="px-3 py-2">
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(pgTxn.status)}`}>
                              {pgTxn.status}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{pgTxn.created_at ? formatDate(pgTxn.created_at) : '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-center py-4">No payment gateway transactions found</p>
              )}
            </div>

            <div className="mb-6">
              <h3 className="font-semibold text-lg text-gray-800 dark:text-white mb-3">LCR Money Transactions</h3>
              {selectedPaymentDetail.lcr_money_transactions && selectedPaymentDetail.lcr_money_transactions.length > 0 ? (
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {selectedPaymentDetail.lcr_money_transactions.map((lm, index) => (
                        <tr key={index} className="border-b border-gray-100 dark:border-gray-800">
                          <td className="px-3 py-2 text-sm font-semibold text-gray-900 dark:text-white">₹{parseFloat(lm.amount).toLocaleString()}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lm.type}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lm.received_from}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lm.received_by}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lm.purpose}</td>
                          <td className="px-3 py-2">
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${lm.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                              {lm.status}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lm.date} {lm.time}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-center py-4">No LCR Money transactions found</p>
              )}
            </div>

            <div>
              <h3 className="font-semibold text-lg text-gray-800 dark:text-white mb-3">LCR Rewards Transactions</h3>
              {selectedPaymentDetail.lcr_rewards_transactions && selectedPaymentDetail.lcr_rewards_transactions.length > 0 ? (
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {selectedPaymentDetail.lcr_rewards_transactions.map((lr, index) => (
                        <tr key={index} className="border-b border-gray-100 dark:border-gray-800">
                          <td className="px-3 py-2 text-sm font-semibold text-gray-900 dark:text-white">₹{parseFloat(lr.amount).toLocaleString()}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lr.type}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lr.received_from}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lr.received_by}</td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lr.purpose}</td>
                          <td className="px-3 py-2">
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${lr.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                              {lr.status}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{lr.date} {lr.time}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-center py-4">No LCR Rewards transactions found</p>
              )}
            </div>
          </div>
        </div>
